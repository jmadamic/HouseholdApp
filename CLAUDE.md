...
- **Multi-member support**: Household supports N members (not fixed to 2). Members stored as JSON array in `@AppStorage("memberNames")`.
- **Multi-select assignment**: Chores and shopping items use `assignedToMembers` String (comma-separated member indices). Empty/nil = Everyone. Set via `assignedMemberIndices: Set<Int>` accessor in CoreDataHelpers. Legacy `assignedTo` Int16 kept for CloudKit backward compat.
- **Filter bars**: "All" + one tab per member (no "Everyone" tab). Filtering shows items with empty `assignedMemberIndices` OR containing the selected member index.
...
- **Member assignment**: `assignedToMembers` String stores comma-separated member indices (e.g. `"0,1"`). Empty/nil = Everyone. Accessed via `assignedMemberIndices: Set<Int>` property on Chore and ShoppingItem. `completedByMembers` String stores comma-separated indices of who completed.
