...
- **Auto-cleanup**: completed chores, purchased items, past meals, trips ended >1 week ago (cascading to their packing items), and fully-harvested plants are deleted once per session on first snapshot (`hasCleanedUp` flag in each store). Saved meals and wishes are long-lived and never auto-cleaned. **Archived meals and trips (`isArchived == true`) are also exempt** — that's the whole point of archiving over deleting.
- **Archiving**: `MealStore`/`TripStore` publish everything in `meals`/`trips` (Settings counts and delete-all rely on that) and expose `activeMeals`/`archivedMeals`, `activeTrips`/`archivedTrips`. Tabs, trip pickers (`MealFormView`, `ChoreFormView`), and the spreadsheet importer's dedupe context must use the **active** lists; use the full list only where archived items should count.
...
- **Docs are part of every push**: update BOTH `README.md` (features table, project structure, data model) and `CLAUDE.md` (conventions, architecture, new gotchas) in the same PR as the code. Not "when structure changes" — every push. CLAUDE.md loads into context each session, so drift there silently misleads future work; it went stale for ~10 features before being caught.
...
- **Spreadsheet import** (`XLSXReader` + `MealPlanImporter` + `MealPlanImportView`): the bundled `Resources/MealPlanTemplate.xlsx` is generated by `scripts/make-meal-template.py` (openpyxl) — **regenerate it and update the importer's header aliases together** when columns change. `XLSXReader` is a deliberate zero-dependency reader (zip central directory + Apple `Compression` raw-deflate + `XMLParser`); it does not support zip64 or encrypted files, which spreadsheet apps never produce at this size. Meals sheet layout is one ingredient per row: a row with blank Date+Meal but an Ingredient is a *continuation* of the meal above; "Need to buy?" is a Yes/No dropdown (`parseYes` also accepts TRUE/✓/x/1); rows whose name starts with "Example" are the template's grey samples and are skipped. The older two-column comma-list layout still imports. The importer never fails a whole file: each row yields documents or an `ImportIssue` with sheet + row, and duplicates against existing meals/items/trips are skipped so re-import is idempotent. Writes happen only from the preview's Import button, in dependency order (trips → meals → shopping → packing).
- **Meal ingredients → shopping**: the cart toggle on an ingredient sets `have = false`; shopping items are created **at save time only** (`createShoppingItems`) and flagged `addedToList`. Never write shopping items mid-edit — cancelling the form would orphan them.
