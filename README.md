...
- An **Apple ID** for code signing — a free personal team works, but apps expire after 7 days and must be re-installed twice a week (see `scripts/refresh-sideload.sh`). A paid Apple Developer membership removes the weekly expiry.
...
├── scripts/refresh-sideload.sh         ← Twice-weekly rebuild + reinstall for free-team signing
├── scripts/com.householdapp.refresh-sideload.plist  ← launchd schedule (copy to ~/Library/LaunchAgents/)
...
With a free personal team the install expires after **7 days** — `scripts/refresh-sideload.sh` rebuilds and reinstalls on all household phones. It runs automatically via launchd on **Sundays and Wednesdays**, every 30 minutes from 8 AM to 10 PM; phones must be unlocked and reachable.

Twice a week rather than weekly because the longest gap is then 4 days, so it takes two consecutive missed days — not one — for the app to expire. Per-device success stamps in `~/Library/Application Support/HouseholdApp/` (2-day TTL, deliberately under the 3-day Sunday→Wednesday gap) make the repeat fires no-ops once a phone has been reached. The schedule lives in `scripts/com.householdapp.refresh-sideload.plist`; edit it there, copy to `~/Library/LaunchAgents/`, then `launchctl unload && launchctl load` it.
